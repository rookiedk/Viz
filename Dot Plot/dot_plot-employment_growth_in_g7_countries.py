...
plt.xkcd();
#Load data
data_source = 'C:\Users\danandak\Desktop\\temp\Employment Growth in G-7 Countries.xlsx'
...
for i in range(0, len(y_labels)):
    ...
    if 'United Kingdom' in y_labels[i]:
        ...
        ax.text(x1[i] - 6, y_pos[i], y_labels[i])
    if i==0 or i==2:
        ax.plot((x1[i], x2[i]), (y_pos[i],  y_pos[i]), color=color, linewidth=1.5,
            label='Employment is greater than Net Employment Growth' if 'green' in color else 'Employment is lesser than Net Employment Growth')
    else:
        ax.plot((x1[i], x2[i]), (y_pos[i], y_pos[i]), color=color, linewidth=1.5)
ax.legend(loc='best')
ax.annotate('Canada has the lowest difference with\nEmployment at 5.1% and Net Employment Growth at 5.2% Net', xy=(5.5, 1), xytext=(15,0.8),arrowprops=dict(arrowstyle="->",
                            connectionstyle="angle3,angleA=0,angleB=-90"))
ax.annotate('USA has the highest difference with\nEmployment at 42.0% and Net Employment Growth at 55.9% Net', xy=(40, 0.2), xytext=(40,1.4),arrowprops=dict(arrowstyle="->",
                            connectionstyle="angle3,angleA=0,angleB=-90"))
ax.set_title('Employment Growth in G-7 Countries\n(Source : Business Insider)')
ax.text(50, 6.2, '')
